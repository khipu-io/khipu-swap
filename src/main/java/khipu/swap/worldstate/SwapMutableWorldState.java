...
public class SwapMutableWorldState implements MutableWorldState {
  ...
  public SwapMutableWorldState(final WorldState worldState) {
    ...
    if (!(worldState instanceof SwapMutableWorldState)) {
      throw new UnsupportedOperationException();
    }

    final SwapMutableWorldState other = (SwapMutableWorldState) worldState;
  ...
  public MutableWorldState copy() {
    return new SwapMutableWorldState(rootHash(), worldStateStorage, preimageStorage);
  }
  ...
  public final boolean equals(final Object other) {
    if (!(other instanceof SwapMutableWorldState)) {
      return false;
    }

    final SwapMutableWorldState that = (SwapMutableWorldState) other;
  ...
  protected static class Updater
      extends AbstractWorldUpdater<SwapMutableWorldState, WorldStateAccount> {

    protected Updater(final SwapMutableWorldState world) {
      super(world);
    }

    @Override
    protected WorldStateAccount getForMutation(final Address address) {
      final SwapMutableWorldState wrapped = wrappedWorldView();
      final Hash addressHash = Hash.hash(address);
      return wrapped
          .accountStateTrie
          .get(addressHash)
